...
def log_results(id,
                ...
                dnn=None,
                labels=None,
                resdir='resdir'):  #, fname, res_dir=config.res_dir):
    ...
    for res in range(0, num):
        ...
        if labels is not None:
            ...
            label = "-"
            correct = "-"
        r.write("{};{};{};{:.4f};{:.4f}".format(correct, label,
                                                predicted_label, prob, ent))
        for cl in range(0, nb_classes):
...
def run(id, cv, data_params, learn_params, model=None):
    ...
    if model is None:
        if learn_params['dnn_type'] == "lstm":
            ...
            model = tor_sdae.build_model(learn_params,
                                         nb_classes,
                                         data_params['train_gen'],
                                         data_params['val_gen'],
                                         steps=(learn_params['train_steps'],
                                                learn_params['val_steps']),
                                         pre_train=False)
    ...
    if learn_params['optimizer'] == "sgd":
        ...
        optimizer = Adam(lr=learn_params['lr'], decay=learn_params['decay'])
    else:  # elif learn_params['optimizer'] == "rmsprop":
        optimizer = RMSprop(lr=learn_params['lr'], decay=learn_params['decay'])
...
def eval_main():
    ...
    log(
        id, 'Loaded data {} test instances {} packets long'.format(
            nb_instances, nb_packets), dnn)
    ...
    if save_res:
        if not openw:
            ...
            log_results(id,
                        "res_{}x{}_{}_{}".format(nb_classes, nb_instances, id,
                                                 dnn),
                        predicted,
                        nb_classes,
                        dnn=dnn,
            ...
            log_results(id,
                        "res_open{}_{}_{}".format(nb_instances, id, dnn),
                        predicted,
                        200,
                        dnn=dnn,
                        resdir="results_" + dnn)
...
def main(save=False, wtime=False):
    ...
    log(
        id, 'Loaded data {} instances for {} classes: '
        '{} traces per class, {} Tor cells per trace'.format(
            nb_instances, nb_classes, nb_traces, nb_cells))
    ...
    if os.path.isfile(log_exp_name):
        ...
        log_exp = open(log_exp_name, "a")
        log_exp.write(";".join([
            "ID", "w", "tr", "tr length", "DNN", "N layers", "lr", "epochs",
            "tr loss", "tr acc", "cv", "test loss", "test acc", "std acc"
        ]) + "\n")
    ...
    for cv in range(1, cross_val + 1):
        ...
        train_gen = DataGenerator(batch_size=batch_size).generate(
            data, labels, ind_train)
        val_gen = DataGenerator(batch_size=batch_size).generate(
            data, labels, ind_val)
        ...
        _, test_acc, test_loss = predict(id,
                                         model,
                                         test_gen,
                                         steps=len(ind_test),
                                         gen=True)
        ...
        if cross_val == 1:
            ...
            log_exp.write(";".join(
                list(
                    map(lambda a: str(a), [
                        id, nb_classes, nb_traces, nb_cells, dnn, nb_layers,
                        lr, nb_epochs, tr_loss, tr_acc, "x", test_loss,
                        test_acc, "x"
                    ]))) + "\n")
            break
    ...
    if cross_val > 1:
        ...
        log_exp.write(";".join(
            list(
                map(lambda a: str(a), [
                    id, nb_classes, nb_traces, nb_cells, dnn, nb_layers, lr,
                    nb_epochs, best_tr_loss, best_tr_acc, cross_val, mean_loss,
                    mean_acc, "+/-" + str(std_acc)
                ]))) + "\n")
    ...
    if False:
        ...
        x10d_test, y10d_test = load_data(
            ...
            maxlen=maxlen)
        _, test_acc, test_loss = predict(id,
                                         best_model, (x10d_test, y10d_test),
                                         batch_size=1)
